...
function eliminarDatosAntiguos(){
    if(document.querySelector('.categorias-elementos')){
        const listado = document.querySelector('.categorias-elementos');
...
function eliminarCategoria(){
    if(document.querySelectorAll('.eliminarCategoria')[0]){
        const iconosEliminar = document.querySelectorAll('.eliminarCategoria');
        iconosEliminar.forEach(icono =>{
            icono.addEventListener('click', e =>{
                e.preventDefault();
                id = icono.dataset.eliminar;
                url = `http://127.0.0.1:8000/categoria/eliminar/${id}/`
                fetch(url,{
                    method: 'GET',
                    headers: {},
                }).then(respuesta=>{
...
async function cargarlistado(mensaje,tipo){
    try {
        url = "http://127.0.0.1:8000/categoria/listadoJson/";
        const resultado = await fetch(url);
        const datos = await resultado.json();
        //Evitar error
        if(!datos.status){
            datos.categorias.forEach(categoria =>{
                trDatos = crearTrDAtos(categoria);
                if(document.querySelector('.categorias-elementos')){
                    const listado = document.querySelector('.categorias-elementos');
...
function crearTrDAtos({id,nombre}){
    ...
    aActualizar.href= `/categoria/editar/${id}/`;
    ...
    aEliminar.classList.add("eliminarCategoria");
